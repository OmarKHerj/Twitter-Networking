...
def create_post(request):
    if request.method == "POST":
        ...
        if length < 0:
            ...
            messages.add_message(request, messages.ERROR, "Maximum post length: 300 characters")
            return HttpResponseRedirect(reverse("index"))
        else:
...
def register(request):
    if request.method == "POST":
        ...
        # Ensure display name field is valid
        if not 0 < len(display_name) <= 25:
            return render(request, "network/register.html", {
        ...
        try:
            user = User.objects.create_user(username=username, email=email, password=password, name=display_name)
            user.save()
